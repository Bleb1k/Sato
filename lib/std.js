export function* range(from, to) {
    if (to ?? 0) while (from < to) yield from++;
    else while (true) yield from++;
}
...
export function* FCIter(...iterables) {
    ...
    if (arrays.length === 0) {
        while (true) {
            const foo = iterables.map((v) => next(v));
            if (foo.every((i) => i === undefined)) break;
            yield foo;
        }
    }
    ...
    if (max > 0) {
        while (i < max) {
            yield iterables.map((v) => next(v, i));
            ++i;
        }
    }
    return;
